        ...
        <div class="card-box bg-white table-responsive pb-2">
          <table id="datatable" class="table table-striped table-bordered table-hover" style="width:100%">
            ...
            <tbody>
              ...
              // Build map of order_id => pending reminder count (for priority)
              $pendingReminders = $db->getPendingReminders();
              $orderReminderMap = [];
              foreach ($pendingReminders as $rem) {
                  $oid = (int)$rem['order_id'];
                  if (!isset($orderReminderMap[$oid])) {
                      $orderReminderMap[$oid] = 0;
                  }
                  $orderReminderMap[$oid]++;
              }

              // Build map of order_id => [pending_count, total_count] for display (includes acknowledged)
              $orderReminderDisplayMap = [];
              $conn = $db->getConnection();
              $remResult = $conn->query("SELECT order_id, SUM(CASE WHEN is_acknowledged = 0 THEN 1 ELSE 0 END) AS pending_count, COUNT(*) AS total_count FROM order_reminders GROUP BY order_id");
              if ($remResult) {
              ...
              // Collect unique orders into an array so we can sort: pending reminders first
              $seen_job_orders = [];
              $ordersData = [];
              ...
              // Sort by status priority then newest created_at
...
<script>
  ...
  $(document).on('click', '.ready-pickup-order', function () {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        processOrderAction(orderId, 'Ready for Pickup');
      }
    });
  });

  // Handle Complete / Cancel actions with confirmation and optional reason
  $(document).on('click', '.complete-order, .cancel-order', function () {
    const orderId = $(this).data('order-id');
    const isComplete = $(this).hasClass('complete-order');
    ...
    if (isComplete) {
        ...
        config = {
            ...
            html: `
                <p>Are you sure you want to mark this order as <strong>Cancelled</strong>?</p>
            ...
            input: 'textarea',
            inputPlaceholder: 'Please enter the reason for cancellation (Required)',
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            const reason = (config.status === 'Cancelled') ? result.value : null;
            processOrderAction(orderId, config.status, reason);
        }
    });
  });
  ...
  function processOrderAction(orderId, status, reason = null) {
      const actionText = (status === 'Completed') ? "An email message will be sent to the office" : "The order status will be updated and an email will be sent.";
      
      showSweetAlert("Please wait", actionText, "info");
