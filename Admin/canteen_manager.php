...
<title><?= $system_info['system_name'] ?> | Canteen Manager</title>
      ...
      <div class="x_content">
        ...
        <div class="modal fade" id="AddNew" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
          <div class="modal-dialog">
            <form id="SaveForm" autocomplete="off" enctype="multipart/form-data">
              ...
              <input type="hidden" class="form-control" name="user_type" id="user_type" value="Canteen Manager" required>

              <div class="modal-content">
                <div class="modal-header bg-light">
                  <h5 class="modal-title" id="exampleModalLabel">New Canteen Manager</h5>
        ...
        <div class="modal fade" id="UpdateModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
          <div class="modal-dialog">
            <form id="UpdateForm" autocomplete="off" enctype="multipart/form-data">
              ...
              <input type="hidden" class="form-control" name="user_type" id="edit_user_type" value="Canteen Manager"
                required>

              <div class="modal-content">
                <div class="modal-header bg-light">
                  <h5 class="modal-title" id="exampleModalLabel">Update Canteen Manager</h5>
                  <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true"><i class="fa fa-times-circle"></i></span>
                  </button>
                </div>
                <div class="modal-body">
                  ...
                  <div class="form-group">
                    <h6><a href="#" class="text-primary">Password (Leave blank to keep current password)</a></h6>
                  </div>
                  <div class="form-group">
                ...
                <div class="modal-footer">
                  <button type="button" class="btn btn-secondary btn-sm" data-dismiss="modal"><i
                      class="fa fa-times"></i> Cancel</button>
                  <button type="submit" class="btn btn-primary btn-sm" id="edit_submit_button"><i
          ...
          <div class="modal-dialog">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="userProfilePictureModalLabel">Canteen Manager Profile</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true"><i class="fa fa-times-circle"></i></span>
                </button>
              </div>
              <div class="modal-body text-center">
                ...
                <p class="text-secondary mb-1" id="userProfileType">Canteen Manager</p>
              </div>
              <div class="modal-footer">
          ...
          <div class="modal-dialog modal-lg">
            <div class="modal-content">
              <div class="modal-header" id="userProfileModalLabel">
                <h5 class="modal-title">Canteen Manager details</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true"><i class="fa fa-times-circle"></i></span>
                </button>
              </div>
                  ...
                  <div class="row">
                    <div class="col-md-4">
                      <div class="text-center mt-3">
                        <img id="userImage" src="avatar.png" alt="User image" class="rounded-circle" width="150"
                          height="150">
                        <h4 class="mt-3" id="UserName">User Name</h4>
                        <p class="text-secondary mb-1">Canteen Manager</p>
                        <p class="text-muted font-size-sm" id="userCreatedat">Date Added</p>
                      </div>
                    </div>
                    <div class="col-md-8">
        ...
        <div class="card-box bg-white table-responsive pb-2">
          ...
          <table id="datatable" class="table table-striped table-bordered table-hover" style="width:100%">
            ...
            <tbody>
              <?php
              $instructor = $db->getUsersByLevel(3);
              while ($row2 = $instructor->fetch_assoc()) {

                ?>
                <tr>
                  <td><input type="checkbox" class="select-record d-block m-auto" value="<?= $row2['user_id'] ?>"></td>
                  <td>
                    <img src="../assets/img/canteen-staff/<?php echo $row2['image']; ?>" alt="" width="35" height="35"
                  ...
                  <td>
                    <button type="button" class="btn btn-info btn-sm view-user" data-user-id="<?= $row2['user_id']; ?>"
                      data-firstname="<?= htmlspecialchars($row2['firstname']); ?>"
                      data-middlename="<?= htmlspecialchars($row2['middlename']); ?>"
                      data-lastname="<?= htmlspecialchars($row2['lastname']); ?>"
                      data-suffix="<?= htmlspecialchars($row2['suffix']); ?>"
...
<script>
  $(document).ready(function () {
    ...
    $('#SaveForm').submit(function (e) {
      e.preventDefault();
      showSweetAlert("Please wait", "An email message will be sent to the newly registered user", "info");
      ...
      $.ajax({
        ...
        success: function (response) {
          if (response.success) {
            showSweetAlert("Success!", response.message, "success", "canteen_manager.php");
          } else {
    ...
    $('#delete-selected').on('click', function () {
      ...
      confirmMultipleDeletion(selectedIDs.length, function () {
        deleteMultipleRecords("tbluser", "user_id", selectedIDs, "canteen_manager.php");
      });
    });
    ...
    $('#datatable').on('click', '.view-profile', function () {
      ...
      const imagePath = image ? '../assets/img/canteen-staff/' + image : 'avatar.png';
    ...
    $('#datatable').on('click', '.view-user', function () {
      ...
      $('#userProfileModal #userImage').attr('src', image ? '../assets/img/canteen-staff/' + image : 'avatar.png');
